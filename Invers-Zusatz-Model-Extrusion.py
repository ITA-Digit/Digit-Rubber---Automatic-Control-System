...
def main():
    ...
    for (columnName, ColumnData) in target.iteritems():
            if columnName != 'index':
                ...
                plt.plot(target_pred['index'], target_pred[columnName],
                         'o', ms=1, c='b', alpha=1, linewidth=3)
                plt.grid(axis='both', linestyle='-', alpha=1.0, color='black')
                ...
                if columnName != 'index':
                    ...
                    print(f"\nTRANING: Trainingsmetrik - {columnName}:\nMSE: {mse_model}\nR2 Score: {r2_score_model}\nMAE: {mae_model}\nMAPE: {mape_model}%")
                    ...
                    print(f"\nPREDICTION: Validierungsmetrik  - {columnNa-me}:\nMSE: {mse_pred}\nR2 Score: {r2_score_pred}\nMAE: {ma-e_pred}\nMAPE: {mape_pred}%")
    ...
    plot_loss(epoch_count, training_loss, validation_loss, ylim=(0, 20))#,xlim=(0, 1000))
