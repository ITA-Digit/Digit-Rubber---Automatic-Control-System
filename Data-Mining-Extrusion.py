...
Seed_1 = 42
MODEL_NAME = 'Model_1'
DATA_FILE = 'Extruder_data_20%_shuffled.csv'
...
def preprocess_data(input_data):
    Stellgroeßen = input_data[['Drehzahl [1/min]', 'T (Schnecke)','T (Stiftzo-ne)','T (Werkzeug)','T (Speisewalze)','T (Einzug)']]
    Messgroeßen = input_data[['T 1 [C]  (0,0 mm)', 'T 2 [C]  (10,0 mm)', 'T 3 [C]  (20,0 mm)', 'T 4 [C]  (30,0 mm)',
                              'T 5 [C]  (40,0 mm)', 'T 6 [C]  (50,0 mm)', 'T 7 [C]  (60,0 mm)', 'T 8 [C]', 'T b', 'T a']]
...
def plot_residuals(predictions, residuals, data_label, color):
    ...
    plt.title(f'Residuum-Plot für {data_label} Data')
    plt.xlabel('Prädiktionswerte')
    plt.ylabel('Residuuen')
    plt.grid(axis='both', linestyle='-', alpha=1.0, color='black')
...
def main():
    ...
    data_train = data.iloc[1:2387] # 80 %
    data_test = data.iloc[2387:2984] # 20 %
    target_train = target.iloc[1:2387] # 80 %
    target_test = target.iloc[2387:2984] # 20 %

    # Ausgabe des Trainings- und Validierungssplit
    print('Groeße des Trainings- und Validierungssplits')
    ...
    early_stopping = EarlyStopping(monitor='val_loss', patience=30, resto-re_best_weights=True)
    
    # ModelCheckpoint callbacks - Speichern des Modells mit der höchsten Genauigkeit
    ...
    model.compile(
        loss=tf.keras.losses.MeanAbsolutePercentageError(),
                  optimizer=tf.keras.optimizers.Adam(learning_rate=0.003688),
                  metrics=['MAPE']
                  )
    ...
    validation_loss = history.history['val_loss']  # Accessing validation loss
    ...
    plot_loss(epoch_count, training_loss, validation_loss, ylim=(0, 5),xlim=(0, 431))
    ...
    plot_residuals(test_predictions, test_residuals, 'Test', 'blue')
